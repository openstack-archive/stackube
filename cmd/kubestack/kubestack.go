...
func (os *OpenStack) getNetworkIDByNamespace(namespace string) (string, error) {
	tenantID, err := os.Client.GetTenantIDFromName(namespace)
	if err != nil {
		return "", err
	}
	// Get openstack networkID by tenantID
	network, err := os.Client.GetOpenStackNetworkByTenantID(tenantID)
	if err != nil {
		return "", err
	}
...
func initOpenstack(stdinData []byte) (OpenStack, string, error) {
	...
	if n.KubestackConfig == "" {
		return OpenStack{}, "", fmt.Errorf("kubestack-config not specific")
	}
	...
	// Init plugin
	if os.Client.PluginName != "" {
		integrationBridge := "br-int"
		if os.Client.IntegrationBridge != "" {
			integrationBridge = os.Client.IntegrationBridge
		}

		plugin, _ := plugins.GetNetworkPlugin(os.Client.PluginName)
		if plugin != nil {
...
func cmdAdd(args *skel.CmdArgs) error {
	...
	// Populate container interface mac address
	conInterface.Mac = port.MACAddress
